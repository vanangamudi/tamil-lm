...
def read_csv(filename='../tace16/data_new.csv'):
...
def test2(string='அம்மா'):
    ...
    print('orig:{}'.format(string), type(string))
    print('orig bytes: {}'.format(string.encode()), type(string.encode()))
    print('orig numbers: {}'.format([i for i in string.encode()]), type([i for i in string.encode()]))
    print('utf_version: {}'.format(utf_version), type(utf_version))
    print('utf_version bytes: {}'.format(bytes(utf_version)), type(bytes(utf_version)))
    print('utf_version bytes: {}'.format(bytes(utf_version).decode('utf-8')), type(bytes(utf_version).decode('utf-8')))
